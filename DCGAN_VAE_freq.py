...
class DCGAN_G(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, out_channels = 256):
        ...
        main.add_module(
            ...
            nn.ConvTranspose2d(cngf, nc * self.out_channels, 4, 2, 1, bias=False),
        )
        self.main = main
    ...
    def forward(self, input):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            output = nn.parallel.data_parallel(self.main, input, range(self.ngpu))
            output = output.view(-1, self.nc, self.out_channels, self.isize, self.isize)
